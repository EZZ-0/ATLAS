...
def generate_investment_summary_pdf(financials, generator, recommendation_data):
    ...
    if business_summary and business_summary != 'No business description available':
        # Truncate to 500 chars for PDF
        summary_text = business_summary[:500] + ('...' if len(business_summary) > 500 else '')
    ...
    def get_ratio(key):
        if not generator.ratios.empty and key in generator.ratios.index:
            return generator.ratios.loc[key].iloc[0]
        return None
    
    # Metrics table
    metrics_data = [
        ['Metric', 'Value'],
        ['Current Price', f"${get_ratio('Current_Price'):.2f}" if get_ratio('Current_Price') else "N/A"],
        ['P/E Ratio', f"{get_ratio('PE_Ratio'):.1f}x" if get_ratio('PE_Ratio') else "N/A"],
        ['Market Cap', f"${get_ratio('Market_Cap')/1e9:.1f}B" if get_ratio('Market_Cap') else "N/A"],
        ['Revenue (TTM)', f"${get_ratio('Revenue')/1e9:.1f}B" if get_ratio('Revenue') else "N/A"],
        ['Revenue CAGR', f"{get_ratio('Total_Revenue_CAGR')*100:.1f}%" if get_ratio('Total_Revenue_CAGR') else "N/A"],
        ['ROE', f"{get_ratio('ROE')*100:.1f}%" if get_ratio('ROE') else "N/A"],
        ['ROIC', f"{get_ratio('ROE')*100:.1f}%" if get_ratio('ROE') else "N/A"],  # Simplified
        ['Debt/Equity', f"{get_ratio('Debt_to_Equity'):.2f}x" if get_ratio('Debt_to_Equity') is not None else "N/A"],
    ]
