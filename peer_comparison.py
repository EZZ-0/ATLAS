...
def discover_peers(ticker: str, max_peers: int = 10) -> Dict:
    ...
    FAST VERSION: Uses pre-built sector mapping to avoid calling yfinance for all 500 tickers
    ...
    try:
        ...
        # Get candidate peers from sector mapping
        candidate_tickers = get_sector_peers(ticker, company_sector, max_peers=max_peers * 3)
        ...
        for peer_ticker in candidate_tickers:
            try:
                ...
                similarity_score = 1.0  # Same sector (guaranteed)
                
                # Bonus for same industry
                if peer_industry == company_industry:
                    similarity_score += 1.0
                
                # Bonus for similar size
                if company_market_cap > 0 and peer_market_cap > 0:
                    ...
                    if size_ratio <= 10:  # Within 10x
                        similarity_score += 0.5
                    if size_ratio <= 3:  # Within 3x (very similar)
                        similarity_score += 0.5
                
                peers.append({
                    'ticker': peer_ticker,
                    'name': peer_info.get('longName', peer_ticker),
                    'sector': company_sector,  # Already know it's same sector
                    'industry': peer_industry,
                    'market_cap': peer_market_cap,
                    'similarity_score': similarity_score
                })
                
                print(f"[OK] Added peer: {peer_ticker} (score: {similarity_score:.1f})")
        ...
        result = {
            ...
            'discovery_method': 'Fast sector mapping + similarity scoring'
        }
